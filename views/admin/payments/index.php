                            ...
                            <table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                                ...
                                <tbody>
                                    <?php foreach ($payments as $payment): ?>
                                        <tr>
                                            <td class="text-primary"><strong><?= htmlspecialchars($payment['booking_id']) ?></strong></td>
                                            <td><?= htmlspecialchars($payment['billplz_id']) ?></td>
                                            <td><?= htmlspecialchars($payment['payment_ref_no']) ?></td>
                                            <td class="action-buttons">
                                                <a href="<?= APP_URL . '/admin/payments/view/' . $payment['id'] ?>" class="btn btn-info  btn-sm"><i class="fas fa-eye"></i>
                                                </a>
                                                <a href="<?= APP_URL . '/admin/payments/delete/' . $payment['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this payment?')"><i class="fas fa-trash"></i>
                                                </a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
